...
func TestArchiveMissDoesNotBlockSavingBookmark(t *testing.T) {
	repository := store.NewMemoryRepository()
	collection, err := domain.NewCollection("collection-001", "Reading")
	if err != nil {
		t.Fatal(err)
	}
	if err := repository.CreateCollection(context.Background(), collection); err != nil {
		t.Fatal(err)
	}
	if _, err := repository.ArchiveBookmark(context.Background(), "bookmark-missing"); err == nil {
		t.Fatal("expected missing bookmark error")
	}
	bookmark, err := domain.NewBookmark("bookmark-001", collection.ID, "https://example.com/article", "Example", []string{"go"})
	if err != nil {
		t.Fatal(err)
	}
	done := make(chan error, 1)
	go func() { done <- repository.SaveBookmark(context.Background(), bookmark) }()
	select {
	case err := <-done:
		if err != nil {
			t.Fatal(err)
	...
	case <-time.After(100 * time.Millisecond):
		t.Fatal("bookmark save remained blocked after archive miss")
	}
}
